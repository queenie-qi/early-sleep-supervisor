...
async function authLogin(req, res) {
  ...
  const users = await sql`SELECT * FROM users WHERE nickname = ${trimmed}`;
  let user = users[0];

  if (!user) {
    const newUsers = await sql`INSERT INTO users (nickname) VALUES (${trimmed}) RETURNING *`;
    user = newUsers[0];
  }
...
async function recordsUpload(req, res) {
  ...
  const users = await sql`SELECT target_time FROM users WHERE id = ${userId}`;
  const user = users[0];
  if (!user) {
    return res.status(404).json({ error: '用户不存在' });
  }

  const groupTargets = await sql`
  ...
  return res.json({ record: records[0], goalAchieved: !!goalAchieved });
}
...
async function recordsToday(req, res) {
  ...
  `;
  return res.json({ record: records[0] || null });
}
...
async function groupsJoin(req, res) {
  ...
  const groups = await sql`SELECT * FROM groups WHERE invite_code = ${inviteCode}`;
  const group = groups[0];
...
async function groupsTargetTime(req, res) {
  ...
  const groups = await sql`
    ...
    RETURNING *
  `;
  return res.json({ group: groups[0] });
}
